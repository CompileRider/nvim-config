...
-- Custom Bare Metal branding with gradient colors
-------------------------------------------------------------------------------
return {
  ...
  config = function()
    -- Gradient highlight groups (gold → orange → red)
    local colors = {
      { "DashboardHeader1", "#FFD700" },
      { "DashboardHeader2", "#FFA500" },
      { "DashboardHeader3", "#FF8C00" },
      { "DashboardHeader4", "#FF6347" },
    }
    ...
    require("dashboard").setup({
      ...
      config = {
        ...
        footer = function()
          ...
          return { "", "🔥 BARE METAL C PROGRAMMING 🔥", "", ("⚡ %d/%d plugins in %.2fms"):format(stats.loaded, stats.count, stats.startuptime) }
        end,
      },
    })
